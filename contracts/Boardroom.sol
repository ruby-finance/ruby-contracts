...
contract Boardroom is ShareWrapper, ContractGuard {
  ...
  function getRubyPrice() external view returns (uint256) {
    return treasury.getRubyPrice();
  }
  ...
  function claimReward() public updateReward(msg.sender) {
    ...
    if (reward > 0) {
      ...
      ruby.safeTransfer(msg.sender, reward);
    ...
    onlyOperator
  {
    ...
    ruby.safeTransferFrom(msg.sender, address(this), amount);
  ...
  function governanceRecoverUnsupported(
    ...
    require(address(_token) != address(ruby), "ruby");
