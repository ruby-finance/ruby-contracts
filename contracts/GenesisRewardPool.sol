...
contract GenesisRewardPool {
  ...
  struct PoolInfo {
    ...
    uint256 allocPoint; // How many allocation points assigned to this pool. RUBY to distribute.
    uint256 lastRewardTime; // Last time that RUBY distribution occurs.
    uint256 accRubyPerShare; // Accumulated RUBY per share, times 1e18. See below.
  ...
  modifier onlyOperator() {
    require(
      operator == msg.sender,
      "RubyGenesisPool: caller is not the operator"
    );
    _;
  }
  ...
  function checkPoolDuplicate(IERC20 _token) internal view {
    ...
    for (uint256 pid = 0; pid < length; ++pid) {
      require(poolInfo[pid].token != _token, "RubyGenesisPool: existing pool?");
    }
  }
  ...
  // View function to see pending RUBY on frontend.
  function pendingRUBY(uint256 _pid, address _user)
    external
    view
    returns (uint256)
  {
    ...
    return user.amount.mul(accRubyPerShare).div(1e18).sub(user.rewardDebt);
  }
  ...
  function deposit(uint256 _pid, uint256 _amount) public {
    ...
    updatePool(_pid);
    if (user.amount > 0) {
      uint256 _pending = user.amount.mul(pool.accRubyPerShare).div(1e18).sub(
        user.rewardDebt
      );
      if (_pending > 0) {
  ...
  function safeRubyTransfer(address _to, uint256 _amount) internal {
    uint256 _rubyBalance = ruby.balanceOf(address(this));
    if (_rubyBalance > 0) {
      if (_amount > _rubyBalance) {
        ruby.safeTransfer(_to, _rubyBalance);
      } else {
